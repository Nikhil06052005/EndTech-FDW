...
function Home() {
  ...
  return (
    <div className="bg-slate-200 w-full min-h-screen ">
      ...
      {!input ? (
        <div className="flex flex-wrap items-center justify-center gap-5 w-[100%] ">
          {Categories.map((item) => {
            return (
              <div
                className="w-[140px] h-[150px] bg-white flex items-start gap-5 p-5 flex-col justify-start text-[20px] font-semibold text-gray-600 rounded-lg shadow-xl hover:bg-green-200 cursor-pointer transition-all duration-200"
      ...
      <div
        className={`w-full md:w-[40vw] h-[100%] fixed top-0 right-0 bg-white shadow-xl rounded-lg p-6 transform transition-transform duration-500 flex flex-col items-center overflow-auto ${showCart ? "translate-x-0" : "translate-x-full"}`}
      >
        ...
        {items.length > 0 ? (
          <>
            <div className="w-full mt-9 flex flex-col gap-8"></div>
            <div>
              {items.map((item) => (
                <Card2
                  name={item.name}
            ...
            <button className="w-[80%] p-3 bg-green-500 rounded-lg text-white hover:bg-green-400 transition-all cursor-pointer" onClick={()=> toast.success("Order Placed Successfully!")}>
              Place Order
            </button>
          </>
        ) : (
